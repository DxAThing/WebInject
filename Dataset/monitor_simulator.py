...
# ============================================================
# 使用 headless 浏览器将 HTML 渲染为截图。
# 假设浏览器原始像素值即为用户实际看到的像素值，
# 不再进行 ICC 色彩空间变换或 U-Net 映射。
#
# render() 返回单张 PIL Image（浏览器原始像素）。
# ============================================================
...
class MonitorSimulator:
    """
    显示器渲染模拟器。
    利用 headless 浏览器将 HTML 页面渲染为指定分辨率的截图。

    假设原始像素值即为用户看到的像素值（弃用 ICC / U-Net）。
    render() 返回单张 PIL Image。
    """
    ...
    # 核心渲染方法
    # ----------------------------------------------------------
    def render(self, html_path: str, monitor_config: dict) -> Image.Image:
        """
        将指定 HTML 文件渲染为浏览器截图。

        假设浏览器原始像素即为用户看到的像素值，
        不再进行 ICC 色彩变换或 U-Net 映射。

        参数:
            html_path      : HTML 文件的绝对路径
            monitor_config : 显示器配置字典，包含 width / height
